...
export abstract class PrismaClientRepository {
  ...
  protected async run<T>(operation: (client: PrismaClient) => Promise<T>): Promise<T> {
    return operation(this.prisma);
  }

  protected async transaction<T>(operation: (client: PrismaClient) => Promise<T>): Promise<T> {
